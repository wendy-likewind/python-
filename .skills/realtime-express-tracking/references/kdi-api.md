...
| Endpoint | `GET https://app-blbnihghgum9-api-AalZz7v4QEGL-gateway.appmiaoda.com/kdi` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json;charset=UTF-8` |
| third_part_domain | `app-blbnihghgum9-api-AalZz7v4QEGL-gateway.appmiaoda.com` |
...
async function queryExpressTracking(no: string, type?: string): Promise<TrackingResult> {
  const url = new URL(
    "https://app-blbnihghgum9-api-AalZz7v4QEGL-gateway.appmiaoda.com/kdi"
  );
