...
| Endpoint | `GET https://app-blbnihghgum9-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/{id}` |
| Auth 模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| third_part_domain | `app-blbnihghgum9-api-o9wN0pyVE2ea-gateway.appmiaoda.com` |
...
async function queryOmniVideoTask(
  ...
  let url: string;
  if (taskId) {
    url = `https://app-blbnihghgum9-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/${taskId}`;
  } else {
    url = `https://app-blbnihghgum9-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/${externalTaskId}?external_task_id=${externalTaskId}`;
  }
...
serve(async (req: Request): Promise<Response> => {
  ...
  let queryUrl: string;
  if (taskId) {
    queryUrl = `https://app-blbnihghgum9-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/${taskId}`;
  } else {
    queryUrl = `https://app-blbnihghgum9-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/${externalTaskId}?external_task_id=${externalTaskId}`;
  }
