...
| Endpoint | `POST https://app-blbnihghgum9-api-V9PworyO6gEa-gateway.appmiaoda.com/train/station2s` |
| 认证方式 | platform_managed（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json;charset=UTF-8` |
| Third-party Domain | `app-blbnihghgum9-api-V9PworyO6gEa-gateway.appmiaoda.com` |
...
async function queryTrainStation2s(
  ...
  const response = await fetch(
    `https://app-blbnihghgum9-api-V9PworyO6gEa-gateway.appmiaoda.com/train/station2s?${params}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
