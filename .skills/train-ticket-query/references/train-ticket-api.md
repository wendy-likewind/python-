...
| Endpoint | `POST https://app-blbnihghgum9-api-DLEO73lBPZ2a-gateway.appmiaoda.com/train/ticket` |
| 认证方式 | platform_managed（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json;charset=UTF-8` |
| Third-party Domain | `app-blbnihghgum9-api-DLEO73lBPZ2a-gateway.appmiaoda.com` |
...
async function queryTrainTicket(
  ...
  const response = await fetch(
    `https://app-blbnihghgum9-api-DLEO73lBPZ2a-gateway.appmiaoda.com/train/ticket?${params}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
