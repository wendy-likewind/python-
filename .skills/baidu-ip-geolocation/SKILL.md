...
- **Endpoint**：`GET https://app-blbnihghgum9-api-79jK62Ze2pQL-gateway.appmiaoda.com/location/ip`
...
async function callIpLocation(
  ...
  const url = `https://app-blbnihghgum9-api-79jK62Ze2pQL-gateway.appmiaoda.com/location/ip${
...
serve(async (req: Request): Promise<Response> => {
  ...
  if (ip) params.set("ip", ip);
  if (coor) params.set("coor", coor);
  const upstreamUrl = `https://app-blbnihghgum9-api-79jK62Ze2pQL-gateway.appmiaoda.com/location/ip${
    params.toString() ? "?" + params.toString() : ""
  }`;
